...
const GOLD_CATEGORIES = ['Chains', 'Rings', 'Bangles', 'Earrings']
        ...
        {product.badges?.length > 0 && (
          <div className="absolute top-3 left-3 flex gap-1 z-10">
            {product.badges.slice(0, 1).map((b, i) => (
              <span key={i} className="badge-gold">{b}</span>
            ))}
          </div>
        )}
        <div className="aspect-square overflow-hidden bg-gray-100">
          <img
            src={product.images?.[0]?.url || 'https://placehold.co/600x600/f5f0eb/b8860b?text=Gold'}
            alt={product.name}
            className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
            onError={e => { e.target.src = 'https://placehold.co/600x600/f5f0eb/b8860b?text=Gold' }}
          />
        </div>
        <div className="p-4">
          ...
          <p className="text-lg font-bold text-amber-700">₹{product.price?.toLocaleString('en-IN')}</p>
        </div>
      </Card>
    </Link>
  </div>
...
const Gold = () => {
  ...
  const activeCategory = searchParams.get('category') || GOLD_CATEGORIES[0]
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      const res = await api.get('/products', {
        params: { metal: 'Gold', category: activeCategory, limit: 20 }
      })
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {/* Hero */}
      <div className="mb-8">
        <p className="text-xs uppercase tracking-[0.25em] text-amber-600/70 mb-2">Premium Collection</p>
        <h1 className="text-4xl font-serif font-bold text-amber-800 mb-3">Gold Jewellery</h1>
